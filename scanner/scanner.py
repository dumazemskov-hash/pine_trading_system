...
# === RAID Hunter v8.29 ===
# Full signal logging for analysis without screenshots
...
def log_signal(signal, ohlcv, atr):
    ...
    recent = [{"ts": c[0], "o": round(c[1], 8), "h": round(c[2], 8), "l": round(c[3], 8), "c": round(c[4], 8), "v": round(c[5], 4)} for c in ohlcv[-CANDLES_TO_LOG:]]
    record = {
...
def get_symbols():
    ...
    symbols = [s for s, m in markets.items() if m.get("swap") and m.get("quote") == "USDT" and m.get("active") and s.endswith(":USDT")]
    print(f"Загружено {len(symbols)} активных монет")
    return symbols

def calc_atr(ohlcv, period=14):
    if len(ohlcv) < period + 1: return None
    trs = [max(ohlcv[i][2]-ohlcv[i][3], abs(ohlcv[i][2]-ohlcv[i-1][4]), abs(ohlcv[i][3]-ohlcv[i-1][4])) for i in range(-period, 0)]
    return sum(trs)/len(trs)

def is_swing_low(ohlcv, i, n=SWING_N):
    low = ohlcv[i][3]
    for k in range(1, n+1):
        if ohlcv[i-k][3] <= low or ohlcv[i+k][3] <= low: return False
    return True

def find_equal_lows(ohlcv, atr):
    if len(ohlcv) < EQL_LOOKBACK + SWING_N*2 + 2: return None
    end, start = -1, -1 - EQL_LOOKBACK
    swings = [{"price": ohlcv[i][3], "volume": ohlcv[i][5], "index": i} for i in range(start+SWING_N, end-SWING_N+1) if is_swing_low(ohlcv, i, SWING_N)]
    ...
    for i in range(len(swings)):
        ...
        for j in range(len(swings)):
            if i == j: continue
            if abs(swings[j]["price"]-swings[i]["price"]) <= tolerance and abs(swings[j]["index"]-swings[i]["index"]) <= MAX_CLUSTER_SPAN and abs(swings[j]["index"]-swings[i]["index"]) >= MIN_TOUCH_DISTANCE:
                if not any(c["index"] == swings[j]["index"] for c in cluster):
                    cluster.append(swings[j])
        if len(cluster) < 2: continue
        idxs = sorted(c["index"] for c in cluster)
        if any(abs(b-a) < MIN_TOUCH_DISTANCE for a,b in zip(idxs, idxs[1:])): continue
        ...
        score = (1 if touches==2 else 2 if touches==3 else 3) + (1 if avg_vol >= vol_ma*1.2 else 0) + (1 if is_fresh else 0)
        if score > best_score:
            best_score = score
            best_zone = {"zone_low": zone_low, "score": score, "touches": touches, "avg_vol": avg_vol}
    if best_zone is None or best_zone["score"] < MIN_ZONE_SCORE: return None
    return best_zone

def calculate_stop(entry, high, atr):
    return min(high + (atr*STOP_ATR_MULT if atr else high*0.005), entry*(1+MAX_RISK_PCT))

def check_signal(symbol, ohlcv_raw):
    if len(ohlcv_raw) < 75: return None
    ohlcv = ohlcv_raw[:-1]
    ...
    if body_pct < MIN_BODY_PCT: return None
    impulse = (high_p - low_p) / low_p * 100 if low_p > 0 else 0
    if impulse < MIN_BODY_PCT * IMPULSE_STRENGTH: return None
    vol_ma = sum(c[5] for c in ohlcv[-21:-1]) / 20
    if volume < vol_ma * VOLUME_RATIO: return None
    vol_recent = sum(c[5] for c in ohlcv[-21:-1]) / 20
    vol_older = sum(c[5] for c in ohlcv[-61:-21]) / 40
    if vol_older <= 0 or vol_recent < vol_older * PRIOR_VOLUME_MULT: return None
    atr = calc_atr(ohlcv)
    ...
    if low_p >= break_level: return None
    if low_p > min(c[3] for c in ohlcv[-SWEEP_LOOKBACK:]): return None
    if not (close_p < open_p and close_p < prev[4]): return None
    if volume < max(prev[5] * CONDITION_D, zone.get("avg_vol", 0) * 1.15): return None
    entry = close_p
    ...
    return {
        ...
        "zone_low": zone["zone_low"], "zone_avg_vol": zone.get("avg_vol"),
