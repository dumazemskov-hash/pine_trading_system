...
# === RAID Hunter v8.28 ===
# На базе v8.27 + точечные усиления качества:
# 1. True liquidity sweep: low сигнала = самый низкий low за последние 10 баров
# 2. Зона должна быть свежей (хотя бы 1 касание в последних 8 барах)
# 3. Condition D усилен: volume >= max(prev * 0.7, zone_avg_vol * 1.15)
# 4. Более информативный Telegram
# Остальное без изменений (score>=2, break 0.25*ATR)
...
CONDITION_D = 0.70          # было 0.6 → чуть жёстче
MAX_RISK_PCT = 0.02
...
BREAK_ATR_MULT = 0.25
STOP_ATR_MULT = 0.35
SWEEP_LOOKBACK = 10         # NEW: low должен быть самым низким за N баров
FRESH_ZONE_BARS = 8         # NEW: зона должна иметь касание в последних N барах
...
def find_equal_lows(ohlcv, atr):
    ...
    for i in range(len(swings)):
        ...
        # NEW: зона без свежего касания — пропускаем
        if not is_fresh:
            continue
...
def check_signal(symbol, ohlcv_raw):
    ...
    # anti-repaint: только закрытая свеча
    ohlcv = ohlcv_raw[:-1]
    ...
    # NEW: True Liquidity Sweep — low сигнала должен быть самым низким за SWEEP_LOOKBACK баров
...
def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] === Scanner v8.28 запущен ===")
    print("Score>=2 | Break 0.25*ATR | True Sweep 10 | Fresh Zone 8 | CondD 0.70 | Stop cap 2%")
