...
# === RAID Hunter v8.33-exp ===
# Confirmation bar: вход после след. свечи если стоп не снят
# + close-strength + cooldown 32:
...
def check_signal(symbol, ohlcv_raw):
    """Рейд = -2, confirm = -1. Вход если confirm не снял стоп."""
    if len(ohlcv_raw) < 76:
        return None
    ohlcv = ohlcv_raw[:-1]
    if len(ohlcv) < 75:
    ...
    vol_ma = sum(c[5] for c in ohlcv_raid[-21:-1]) / 20
    if volume < vol_ma * VOLUME_RATIO:
        return None

    vol_recent = sum(c[5] for c in ohlcv_raid[-21:-1]) / 20
    vol_older = sum(c[5] for c in ohlcv_raid[-61:-21]) / 40
    if vol_older <= 0 or vol_recent < vol_older * PRIOR_VOLUME_MULT:
        return None

    atr = calc_atr(ohlcv_raid)
    zone = find_equal_lows(ohlcv_raid, atr)
    if zone is None:
        return None
    ...
    recent_lows = [c[3] for c in ohlcv_raid[-SWEEP_LOOKBACK:]]
    ...
    stop = calculate_stop(close_p, high_p, atr)
    if stop <= close_p:
        return None
...
def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] === Scanner v8.33-exp запущен ===")
    print("v8.33 | confirm-bar | body 4-9% | close-strength | cd=32 | full log")
    ...
    while True:
        for symbol in symbols:
            try:
                ...
                send_telegram(msg)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] SIGNAL → {symbol} | score={signal['zone_score']}")
