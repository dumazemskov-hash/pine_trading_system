#!/usr/bin/env python3
"""
DUMP-LONG final check — 90d IS/OOS
Фокус: long_cont и вариации reclaim. Live не трогаем.
"""
...
VARIANTS = {
    "short_base": dict(side="short", mode="base"),
    "long_f3": dict(side="long", mode="fixed", stop_pct=0.03),
    "long_cont": dict(side="long", mode="cont", stop="under_low", max_stop_pct=0.12),
    "long_cont_f3": dict(side="long", mode="cont", stop="fixed", stop_pct=0.03),
    "long_cont_hi": dict(side="long", mode="cont", stop="under_low", max_stop_pct=0.08),
    "long_cont2": dict(side="long", mode="cont2", stop="under_low", max_stop_pct=0.12),
}
...
def build(ohlcv, sig, cfg):
    i = sig["i"]
    side = cfg["side"]
    mode = cfg["mode"]

    if side == "short" and mode == "base":
        entry = sig["c"]
        stop = entry * 1.03
        risk = stop - entry
        return {
            "ts": sig["ts"], "entry": entry, "stop": stop,
            "tp1": entry - risk * TP1_RR, "tp2": entry - risk * TP2_RR,
            "risk": risk, "bar_index": i, "stop_pct_px": 3.0, "side": "short",
        }
    ...
    if mode == "fixed":
        ...
        # 2 bars reclaim: close of i+2 > signal close, and i+1 not deep break
        if i + 2 >= len(ohlcv) - 1:
            return None
        if ohlcv[i + 1][4] <= sig["l"]:
            return None  # washed out
        nc = ohlcv[i + 2][4]
        if nc <= sig["c"]:
            return None
        entry = nc
    ...
    return {
        "ts": sig["ts"], "entry": entry, "stop": stop,
        "tp1": entry + risk * TP1_RR, "tp2": entry + risk * TP2_RR,
        "risk": risk, "bar_index": start,
        "stop_pct_px": risk / entry * 100, "side": "long",
    }
...
def run_variant(name, cfg, by_sym):
    ...
    lines = [
        "=" * 64,
        f"SUMMARY | LONG2 {name}",
        f"90d | {cfg.get('side')} {cfg.get('mode')} | live untouched",
        "=" * 64,
    ]
...
def main():
    print("=" * 64)
    print("DUMP-LONG final | 90d | short vs long_cont")
    print("=" * 64)
    symbols = top_symbols()
    by_sym = {}
    for idx, symbol in enumerate(symbols, 1):
        try:
            ohlcv = fetch_ohlcv_full(symbol, min(BARS_LIMIT, 5000))
        except Exception as e:
    ...
    all_lines = [
        f"DUMP-LONG2 90d | {datetime.now(timezone.utc).strftime('%Y-%m-%d_%H%M')} UTC",
        f"symbols={len(by_sym)}",
        "",
    ]
    ...
    path = out / f"bt_DUMP_LONG2_{stamp}.txt"
    latest = out / "latest_dump_long2.txt"
    text = "\n".join(all_lines) + "\n"
