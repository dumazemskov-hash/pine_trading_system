#!/usr/bin/env python3
"""
DUMP long-run lab — 120d (~4 months) IS/OOS
Только удачные гипотезы из today-lab. Live не трогаем.
"""
...
VARIANTS = {
    ...
    "cpos20_asia_nocasc": dict(cpos_max=0.20, skip_asia=True, no_cascade=True),
}
...
def run_variant(name, cfg, by_sym):
    ...
    lines = [
        "=" * 64,
        f"SUMMARY | LONGRUN {name}",
        f"cpos<={cfg['cpos_max']} asia={cfg['skip_asia']} nocasc={cfg['no_cascade']} | 120d",
        "=" * 64,
    ]
...
def main():
    print("=" * 64)
    print("DUMP long-run | 120d (~4 months) | top hypotheses")
    ...
    for idx, symbol in enumerate(symbols, 1):
        try:
            ohlcv = fetch_ohlcv_full(symbol, min(BARS_LIMIT, 8000))
        except Exception as e:
    ...
    all_lines = [
        f"DUMP longrun 120d | {datetime.now(timezone.utc).strftime('%Y-%m-%d_%H%M')} UTC",
    ...
    path = out / f"bt_DUMP_LONGRUN_{stamp}.txt"
    latest = out / "latest_dump_longrun.txt"
    text = "\n".join(all_lines) + "\n"
