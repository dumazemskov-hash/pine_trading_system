#!/usr/bin/env python3
"""
VAL-FADE lab — отдельная стратегия, НЕ DUMP.
Live dump_scanner.py НЕ трогаем.

После широкого пампа строим грубый профиль (20 корзин на OHLCV),
шорт от отбоя в LVN/премиум в ближайший HVN.

Варианты:
  w8_tight   окно 8,  стоп = high отбоя
  w8_struct  окно 8,  стоп = high импульса
  w16_tight  окно 16, стоп = high отбоя
  w16_struct окно 16, стоп = high импульса

Риск в BT: 1% (не 2% DUMP).
Запуск: python scanner/backtest_val_fade.py
Отчёт:  backtests/bt_VAL_FADE_*.txt + latest_val_fade.txt
"""
...
VARIANTS = {
    "w8_tight": dict(window=8, stop="tight"),
    "w8_struct": dict(window=8, stop="struct"),
    "w16_tight": dict(window=16, stop="tight"),
    "w16_struct": dict(window=16, stop="struct"),
}
...
def build_profile(ohlcv, start, end):
    ...
    while k < N_BINS:
        if bins[k] >= peak * HVN_FRAC:
            ...
            a = lo + s * width
            b = lo + (e + 1) * width
            clusters.append({"lo": a, "hi": b, "mid": (a + b) / 2, "vol": sum(bins[s:e + 1])})
        k += 1
    if len(clusters) < 2:
        return None
    clusters.sort(key=lambda c: c["mid"])
    return {"lo": lo, "hi": hi, "width": width, "clusters": clusters,
            "upper": clusters[-1], "lower": clusters[0], "peak": peak, "bins": bins}


def fade_bar(ohlcv, i, prof):
...
def build_trade(ohlcv, i, cfg):
    w = cfg["window"]
    if i < w + 5 or i >= len(ohlcv) - 3:
        return None, "NOBAR"
    prof = build_profile(ohlcv, i - w, i)
    if not prof:
        return None, "NOPROF"
    if not fade_bar(ohlcv, i, prof):
        return None, "NOFADE"
    entry = ohlcv[i][4]
    if cfg["stop"] == "tight":
    ...
    return {"entry": entry, "stop": stop, "tp1": tp1, "tp2": tp2,
            "risk": risk, "bar_index": i, "stop_pct_px": risk / entry * 100,
            "r1": r1, "r2": r2, "hi": prof["hi"]}, "TAKE"
...
def summarize(raw, label):
    ...
    lines += [
        ...
        f"avg_stop={avg_stop:.2f}% avg_TP1={avg_r1:.2f}R avg_TP2={avg_r2:.2f}R",
...
def run_variant(name, cfg, by_sym):
    ...
    for symbol, ohlcv in by_sym.items():
        ...
        for i in range(80, n - 12):
            ...
            raw_all.append({
                "ts_ms": ohlcv[i][0], "result": res, "bars": bars, "r": r,
    ...
    skip_bits = " ".join(f"{k}={funnel[k]}" for k in ("NOPROF", "NOFADE", "WIDE", "NOTP", "TINY") if funnel[k])
    lines = [
        "=" * 64,
        f"SUMMARY | VAL-FADE {name}",
        f"cfg={cfg} | 60d | risk 1% | DUMP live untouched",
        f"funnel scan={funnel['scan']} TAKE={funnel['TAKE']} {skip_bits}",
        "=" * 64,
    ]
...
def main():
    print("=" * 64)
    print("VAL-FADE lab | pump + HVN/LVN fade | NOT dump")
    ...
    all_lines = [
        f"VAL-FADE | {datetime.now(timezone.utc).strftime('%Y-%m-%d_%H%M')} UTC",
        f"Days={LOOKBACK_DAYS} | symbols={len(by_sym)} | risk={RISK_PCT*100:.0f}%",
        "Отдельная стратегия. dump_scanner.py не менялся.",
        "Профиль: 20 корзин, HVN>=70% пика, нужно >=2 кластера.",
        "Вход: медвежий close над верхним HVN, high не выше high импульса.",
        "",
    ]
    ...
    path = out / f"bt_VAL_FADE_{stamp}.txt"
    latest = out / "latest_val_fade.txt"
    text = "\n".join(all_lines) + "\n"
